...
KMCSpaceFS_UUID boot_uuid; // initialized to 0
...
static bool get_system_root()
{
    ...
    static const WCHAR arc_cspacefs_prefix[] = L"\\ArcName\\CSpaceFS(";
    ...
    while (true)
    {
        ...
        if (!second_time && target.Length >= sizeof(boot_device) - sizeof(WCHAR) && RtlCompareMemory(target.Buffer, boot_device, sizeof(boot_device) - sizeof(WCHAR)) == sizeof(boot_device) - sizeof(WCHAR))
        {
            ExFreePool(target.Buffer);
    ...
    if (target.Length >= sizeof(arc_cspacefs_prefix) - sizeof(WCHAR) && RtlCompareMemory(target.Buffer, arc_cspacefs_prefix, sizeof(arc_cspacefs_prefix) - sizeof(WCHAR)) == sizeof(arc_cspacefs_prefix) - sizeof(WCHAR))
    {
        WCHAR* s = &target.Buffer[(sizeof(arc_cspacefs_prefix) / sizeof(WCHAR)) - 1];
...
static void mountmgr_notification(KMCSpaceFS_UUID* uuid)
{
    ...
    mmtn->DeviceNameLength = sizeof(KMCSPACEFS_VOLUME_PREFIX) + (36 * sizeof(WCHAR));

    RtlCopyMemory(mmtn->DeviceName, KMCSPACEFS_VOLUME_PREFIX, sizeof(KMCSPACEFS_VOLUME_PREFIX) - sizeof(WCHAR));

    w = &mmtn->DeviceName[(sizeof(KMCSPACEFS_VOLUME_PREFIX) / sizeof(WCHAR)) - 1];
...
static void check_boot_options()
{
    ...
    try
    {
        ...
        Status = ZwQueryValueKey(control, &name, KeyValueFullInformation, kvfi, kvfilen, &kvfilen);
        if (!NT_SUCCESS(Status))
        {
...
void boot_add_device(DEVICE_OBJECT* pdo)
{
    ...
    mountmgr_notification(&pdode->KMCSFS.uuid);
}
...
void check_system_root()
{
    ...
    while (le != &pdo_list)
    {
        ...
        if (RtlCompareMemory(&pdode->KMCSFS.uuid, &boot_uuid, sizeof(KMCSpaceFS_UUID)) == sizeof(KMCSpaceFS_UUID))
        {
            if (!pdode->vde)
            {
                pdo_to_add = pdode->pdo;
            }
            else if (pdode->vde->device && !(pdode->vde->device->Flags & DO_SYSTEM_BOOT_PARTITION))
            { // AddDevice has beaten us to it
                NTSTATUS Status;
