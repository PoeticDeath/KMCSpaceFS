...
static NTSTATUS bus_query_hardware_ids(PIRP Irp)
{
    ...
    static const WCHAR ids[] = L"ROOT\\CSpaceFS\0";
...
static NTSTATUS pdo_query_device_id(pdo_device_extension* pdode, PIRP Irp)
{
    ...
    for (i = 0; i < 16; i++)
    {
        *noff = hex_digit(pdode->KMCSFS.uuid.uuid[i] >> 4); noff++;
        *noff = hex_digit(pdode->KMCSFS.uuid.uuid[i] & 0xf); noff++;
...
static NTSTATUS pdo_query_hardware_ids(PIRP Irp)
{
    ...
    static const WCHAR ids[] = L"KMCSpaceFSVolume\0";
